function Get-FileSize {
    Write-Verbose "Evaluate: $itemfullpath" -Verbose
    # Check file if exceeds 100 MB
...
function Get-ProjectFolderFileSize ($folders) {
    #Retrieve List Filename FullPath from a project - Change to Function
    $itemEvaluate = 0
    $fileNameCount = 0
        ...
        if($proj_item -eq ""){
            ...
            if ($proj_item.StartsWith("$/")) {
                ...
                if ($itemfullpath.Contains("/Main/") -or $itemfullpath.Contains("/MAIN/")) {
                    if (Get-FileSize $itemfullpath -eq "y") {
...
function Get-ProjectFolderBinaries ($folders) {
    $binaryFiles = @()
    foreach ($proj_item in $folders) {
        if ($proj_item -eq "") {
            # Skipping blank line
...
foreach ($item in $items) {
    if($item.description -notmatch "retired"){
        if ($excludeProj -contains $item.name) {
            ...
            Get-ProjectFolderFileSize $folders
            $binariesList += Get-ProjectFolderBinaries $folders
